...
const ApproximateAddress = ({longitude, latitude, addressStateUpdate}) => {
    ...
    Geocoder.geocodePosition({lng: longitude, lat: latitude})
        .then(res => setApproxAddress(res[0].formattedAddress, addressStateUpdate(res[0].formattedAddress)))
        .catch(err => {
